...
const StickerPeel = ({
  ...
  const rawId = useId();
  const filterIdSuffix = useMemo(() => rawId.replace(/[^a-zA-Z0-9]/g, ''), [rawId]);
  const pointLightId = `pointLight-${filterIdSuffix}`;
  const pointLightFlippedId = `pointLightFlipped-${filterIdSuffix}`;
  const dropShadowId = `dropShadow-${filterIdSuffix}`;
  const expandAndFillId = `expandAndFill-${filterIdSuffix}`;
  ...
  useEffect(() => {
    ...
    const boundsEl = typeof window !== 'undefined' ? document.body : target.parentNode;
    ...
    const handleResize = () => {
      if (draggableInstanceRef.current && boundsEl) {
  ...
  useEffect(() => {
    const updateLight = e => {
      ...
      if (!rect || !pointLightRef.current) return;
  ...
  return (
    <div className={`draggable ${className}`} ref={dragTargetRef} style={cssVars}>
      <svg width="0" height="0" className="absolute w-0 h-0 pointer-events-none" style={{ position: 'absolute', width: 0, height: 0 }}>
        <defs>
