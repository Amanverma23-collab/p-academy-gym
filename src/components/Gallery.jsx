...
export default function Gallery({ onOpenBooking }) {
  ...
  // Lock body scroll and pause Lenis when lightbox modal is open
  useEffect(() => {
  ...
  return (
    <section 
      id="gallery" 
      className="relative bg-white text-zinc-900 py-16 sm:py-20 lg:py-24 overflow-hidden select-none border-t border-zinc-100 scroll-mt-24"
    >
      {/* Subtle ambient light glow */}
      <div 
        className="absolute left-1/2 top-1/3 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] rounded-full pointer-events-none -z-0 opacity-60"
        style={{
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
              {filteredItems.map((item, idx) => (
                <motion.div
                  ...
                  className={`group relative rounded-2xl sm:rounded-3xl overflow-hidden bg-zinc-100 border border-zinc-200 hover:border-zinc-400 cursor-pointer shadow-xs hover:shadow-xl transition-all duration-300 ${item.span}`}
                >
                  {/* Image */}
                  <img
                    src={item.image}
              ...
              <AnimatePresence mode="popLayout">
                {filteredItems.map((item, idx) => (
                  <motion.div
                    ...
                    onClick={() => setLightboxIndex(idx)}
                    className="group relative rounded-2xl overflow-hidden bg-zinc-100 border border-zinc-200 hover:border-zinc-400 aspect-[4/3] cursor-pointer shadow-xs hover:shadow-xl transition-all duration-300"
                  >
                    <img
                      src={item.image}
                      alt={item.title}
